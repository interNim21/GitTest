...
function check1(btn) {
    ...
    // 버튼의 배경색을 얻어와 저장
    const bgColor = btn.style.backgoundColor;
    ...
    if(bgColor == "yellow") {
        btn.style.backgoundColor = "pink";
    }else {
        btn.style.backgoundColor = "yellow";
    }
}
...
//아이디가 test1-1인 요소를 얻어와 test1a 라는 변수에 저장
...
test1c.onclick = function() {
    test1c.style.backgoundColor = "red";
}
...
// #test2 요소 클릭하면 클릭 횟수 카운트
let count = 0;

test2.addEventListener("click,", function(){
    count++; // 카운트 1증가

    // 표준 이벤트 모델의 이벤트 핸들러 안에 this
    // == 이벤트가 발생한 요소
...
const input3 = document.querySelector("#input3")
    ...
    input3.addEventListener("keyup", function (e){
        ...
        if(e.key == "Enter"){
            box3.style.backgoundColor = input3.value;
        }
...
box3.addEventListener("click", function(){
    ...
    //e.target : 이벤트가 발생한 대상 요소 (이벤틀 발생 요소)

    alert(`배경색 : ${e.target.box3.style.backgoundColor}`);
});