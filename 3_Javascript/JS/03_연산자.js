...
function plusFn() {
    //input요소.value : input 요소의 작성된 값 얻어오기 
    ...
    // -> input 요소에 작성된 값 무조건 문자열(string) 형태라
...
function minusFn() {
    ...
    console.log("두 수의 빼기 :", value1 - value2);

    console.log("두 수의 빼기 : ", Number(value1) - Number(value2));
    result.innerText = Number(value1) - Number(value2);
...
function divFn() {
    ...
    console.log("두 수의 나누기 : ", Number(value1) / Number(value2));
    result.innerText = Number(value1) / Number(value2);
...
function check() {
    // 함수 밖, 안은 구분되는 공간으로 생각하자.
    // 밖에 작성한 변수 count, 안에 작성한 count는 서로 다른 변수(이름만같다)
    let count = 100;
    ...
    // -> 다른 연산보다 먼저 실행된다.
    // -> count 값이 먼저 증가, 혹은 감소된 후 console에 출력
    ...
    console.log("count++ :", count++); // 50 출력 후 51로 증가한다
